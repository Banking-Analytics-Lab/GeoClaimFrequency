# Multimodal (ResNet18 + tabular) with extended cross validation (running in every job one configuration)
...
class MultimodalPoissonNet(nn.Module):
    ...
    Fusion head: concat(image_features, tabular) to MLP to linear predictor.
    Output: exp(eta + log_offset), this is Poisson mean.
    """
...
def main():
    ...
    ap.add_argument("--weights_path", required=True,
        help="Path to resnet18_weights.pth .")
