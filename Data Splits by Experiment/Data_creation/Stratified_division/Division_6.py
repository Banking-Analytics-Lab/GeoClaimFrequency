...
# Create quantile bins for stratification q=10 gives deciles of nclaims
n_bins = 10
df['nclaims_bin'] = pd.qcut(y, q=n_bins, duplicates='drop')
df['nclaims_bin_code'] = df['nclaims_bin'].cat.codes

# Create 6 disjoint stratified folds
...
# Save each fold subset D1...D6
for fold in range(6):
    fold_df = df[df.fold == fold].drop(columns=['nclaims_bin']) 
